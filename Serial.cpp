...
Serial::Serial(uart_port_t uartNo, unsigned int gpioRx, unsigned int gpioTx, int baudRate, unsigned int bufferSize)
{
    miBaudRate = baudRate;
    muiUartNo = uartNo;
...
bool Serial::ReadIntoBuffer()
{
    ...
    while (!muiBufferLen)
    {
        int len = uart_read_bytes(muiUartNo, mpBuffer, muiRxBufferSize, 250 / portTICK_PERIOD_MS);
        if (len < 0)
        {
...
bool Serial::ReadIntoBuffer(unsigned int timeoutms)
{
    ...
    int len = uart_read_bytes(muiUartNo, mpBuffer, maxBytesToRead, timeoutms / portTICK_PERIOD_MS);
    if (len < 0)
    {
        ESP_LOGE(tag, "Error reading from serial interface #%d", muiUartNo);
        return false;
